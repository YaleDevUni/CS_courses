...
node_t *add_inorder(node_t *list, node_t *new)
{
    ...
    for (curr = list; curr != NULL; curr = curr->next) {
        char* curr_time_str = curr->val->start_date;
        char* new_time_str = new->val->start_date;
        int curr_time,new_time;
        int curr_year,curr_mon,curr_day,curr_hour,curr_min,curr_sec;
        int new_year,new_mon,new_day,new_hour,new_min,new_sec;
        sscanf(curr_time_str,"%4d%2d%2dT%2d%2d%2d",&curr_year,&curr_mon,&curr_day,&curr_hour,&curr_min,&curr_sec); 
        sscanf(new_time_str,"%4d%2d%2dT%2d%2d%2d",&new_year,&new_mon,&new_day,&new_hour,&new_min,&new_sec);
        curr_time = (curr_year-2000)*518400+curr_mon*43200+curr_day*1440+curr_hour*60+curr_min; //change string time to minuit based integer 
        new_time = (new_year-2000)*518400+new_mon*43200+new_day*1440+new_hour*60+new_min;
        if (new_time>curr_time) {   // compare new node and current node 
            prev = curr;            // if new node time value greater than current than previous = current node
        } else {
            break;
        }
    }

    new->next = curr;    //link new.next and curr position

    if (prev == NULL) {
        return (new);    //case for new node value smallest
    } else {
        prev->next = new;//else link previous node to new node
        return list;
    }
    return list;
}

...
{   
    ...
    event_t *temp = (event_t *)emalloc(sizeof(event_t)); //memory allocating for event_t 
    strncpy(temp->description, event->description, SUMMARY_LEN); //copy all value from event to temp
    strncpy(temp->end_date, event->end_date, DATE_LEN);
    strncpy(temp->location, event->location, LOCATION_LEN);
    strncpy(temp->start_date, event->start_date, DATE_LEN);
    strncpy(temp->repetition.until, event->repetition.until, DATE_LEN);
    temp->repetition.byday = event->repetition.byday;
    temp->repetition.freq = event->repetition.freq;
    temp->repetition.wkst = event->repetition.wkst;
    return temp;                                    // return copied and allocated value 
